...
def parse_args():
    ...
    parser.add_argument('--savepath', type=str, default=base_path + './Savings/results.json', help='Model save path')
...
if __name__ == '__main__':
    ...
    output_dict_list = [{"input": input_sentences[i],
                         "EDU_breaks": all_segmentation_pred[i],
                         "tree_parsing_output": all_tree_parsing_pred[i]} for i in range(len(input_sentences))]
    
    with open(save_path, 'w', encoding='utf-8') as f:
        json.dump(output_dict_list, f)