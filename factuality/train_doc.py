from raemodel import RAEDocModel, InputFeatures, convert_examples_to_features_longformer, load_and_cache_examples, set_random_seed
...
def evaluate(model, data_path, feature_path, max_length, tokenizer, shuffle):
    ...
    with torch.no_grad():
        ...
        for data in dataloader:
            ...
            if pred.squeeze() == label.squeeze():
                correct += 1
            total += 1
            if label.squeeze() == 1:
                true_total += 1
            if pred.squeeze() == label.squeeze() and label.squeeze() == 1 :
                true_correct += 1
        acc = correct / total
