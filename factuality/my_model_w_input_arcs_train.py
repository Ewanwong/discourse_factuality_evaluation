...
def evaluate(model, eval_dataloader, loss_fn):
    ...
    with torch.no_grad():
        for data in tqdm(eval_dataloader):
            ...
            sent_ids, context_arc_ids, input_ids, input_attention_mask, input_token_ids, input_child_start_indices, input_child_end_indices, input_head_start_indices, input_head_end_indices, context_child_start_indices, context_child_end_indices, context_head_start_indices, context_head_end_indices, dep_labels, input_arcs, context_arcs, sentence_label = data

            output = model(input_ids, input_attention_mask, input_token_ids, input_child_start_indices, input_child_end_indices, input_head_start_indices, input_head_end_indices, input_arcs, context_child_start_indices, context_child_end_indices, context_head_start_indices, context_head_end_indices, context_arcs)
            ...
            sent_ids, context_arc_ids, pred, labels, pos_probs = sent_ids.to('cpu').tolist(), context_arc_ids.to('cpu').tolist(), pred.to('cpu').tolist(), dep_labels.to('cpu').tolist(), pos_probs.to('cpu').tolist()
            all_sent_ids += sent_ids
            all_context_arc_id += context_arc_ids
            all_preds += pred
            all_labels += labels
    ...
    result_dict = unbalanced_and_balanced_sent_accuracy(all_sent_ids, all_context_arc_id, all_preds, all_labels)
...
def unbalanced_and_balanced_sent_accuracy(sent_ids, all_context_ids, pos_probs, labels):  
    assert len(pos_probs) == len(labels)
    assert len(sent_ids) == len(labels)
    assert len(all_context_ids) == len(labels)

    pred_dict = {}

    for sent_id, context_id, pred, label in zip(sent_ids, all_context_ids, pos_probs, labels):
        if sent_id not in pred_dict.keys():
            pred_dict[sent_id] = {}
            pred_dict[sent_id]['context_ids'] = []
            pred_dict[sent_id]['preds'] = []
            pred_dict[sent_id]['labels'] = []
        pred_dict[sent_id]['context_ids'].append(context_id)
        pred_dict[sent_id]['preds'].append(pred)
        pred_dict[sent_id]['labels'].append(label)
    
    result_dict = {} #TODO: 先max
    for sent_id, v in pred_dict.items():
        assert sum(v['labels'])==0 or sum(v['labels'])==len(v['labels'])
        pred_dict[sent_id]['label'] = v['labels'][0]
        pred_dict[sent_id]['pred'] = []
        context_nums  = len(list(set(v['context_ids'])))
        pred_dict[sent_id]['pred'] = [max([v['preds'][i] for i in range(len(v['preds'])) if v['context_ids'][i] == k]) for k in range(context_nums)]
        pred_dict[sent_id]['pred'] = sum(pred_dict[sent_id]['pred']) / len(pred_dict[sent_id]['pred'])
...
if __name__ == '__main__':
    ...
    log_file = f'{data}_baseline_entity_lr_1e-6_bsz_8.txt'
