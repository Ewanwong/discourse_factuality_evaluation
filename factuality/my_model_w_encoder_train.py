...
from my_model_w_encoder import RAEBaseModel
...
def evaluate(model, eval_dataloader, loss_fn):
    ...
    arc_acc, arc_balanced_acc = unbalanced_and_balanced_arc_accuracy(all_preds, all_labels)
    result_dict = unbalanced_and_balanced_sent_accuracy(all_sent_ids, all_preds, all_labels)
    avg_loss = sum(losses) / len(losses)
    #print(all_preds)
    #print(all_labels)
    return avg_loss, arc_acc, arc_balanced_acc, result_dict
        ...
        for epoch in range(epochs):
            ...
            for data in tqdm(train_dataloader):
                ...
                if step % eval_steps == 0:
                    ...
                    avg_loss, arc_acc, arc_balanced_acc, result_dict = evaluate(model, eval_dataloader, loss_fn)
                    print("eval loss:")
                    print(avg_loss)
                    print("eval arc accuracy:")
                    print(arc_acc, arc_balanced_acc)
                    ...
                    print(arc_acc, arc_balanced_acc, file=f)
                    ...
                    avg_loss, arc_acc, arc_balanced_acc, result_dict = evaluate(model, test_dataloader, loss_fn)
                    print("test loss:")
                    print(avg_loss)
                    print("test accuracy:")
                    print(arc_acc, arc_balanced_acc)
...
if __name__ == '__main__':
    ...
    train_data_path = f'../data/{data}_generation_rst_parsed_train.json'
    train_feature_path = f'../data/my_model_{data}_generation_rst_parsed_train.pt'
    eval_data_path = f'../data/{data}_generation_rst_parsed_dev.json'
    eval_feature_path = f'../data/my_model_{data}_generation_rst_parsed_dev.pt'
    test_data_path = f'../data/{data}_human_rst_parsed_test.json'
    test_feature_path = f'../data/my_model_{data}_human_rst_parsed_test.pt'

    eval_save_path = f'../w_encoder_{data}_eval_best'
    test_save_path = f'../w_encoder_{data}_test_best'

    log_file = f'{data}_w_encoder_lr_5e-5_bsz_8.txt'
    ...
    lr_scheduler = get_scheduler(
            name="linear", optimizer=optimizer, num_warmup_steps=500, num_training_steps=5 * train_num
        )

    train(model, train_dataloader, eval_dataloader, test_dataloader, eval_save_path, test_save_path, log_file, optimizer, lr_scheduler=lr_scheduler, loss_fn=loss_fn, epochs=5, eval_steps=100)