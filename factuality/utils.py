...
def convert_examples_to_features_bert(example_dict, tokenizer, max_length=512, pad_token=0, pad_token_segment_id=None,
                                      ):
    """
    每次一个句子
    ...
    for type in ['input_tree', 'context_tree']:
        ...
        tokens_input = []
        tokens_input.extend(tokenizer.encode('<s>', add_special_tokens=False))
        index_now = len(tokens_input)
        for (word_index, word) in enumerate(example['input'].split(' ')):
            ...
            if len(word_tokens) > 0:
                ...
                if type == 'input_tree':
                    index_map[word_index] = index_now - 1
        if type == 'input_tree':
            index_map[word_index+1] = index_now-1

        tokens_input.extend(tokenizer.encode('</s></s>', add_special_tokens=False))
        index_now += len(tokenizer.encode('</s></s>', add_special_tokens=False))
        

        input_length = len(example['input'].split(' '))
        ...
        for (word_index, word) in enumerate(example['context'].split(' ')):
            ...
            if len(word_tokens) > 0:
                tokens_input.extend(word_tokens)
                index_now += len(word_tokens)
                if type == 'context_tree':
                    index_map[word_index+input_length] = index_now - 1
        tokens_input.extend(tokenizer.encode('</s>', add_special_tokens=False))
        if type == 'context_tree':
            index_map[word_index+input_length+1] = index_now-1
        ...
        for i in range((len(example.keys())-3)//4):
            ...
            arc_label_ids = tokenizer.encode(example['dep' + str(i)])
            arc_label_lengths.append(len(arc_label_ids) - 1)
            arc_labels.append(pad_1d(arc_label_ids, 10, pad_token))
        ...
        token_ids = inputs['token_type_ids']
        #################################
        if len(tokens_input)!= len(input_ids): 
            print(len(tokens_input), len(input_ids))
            print([tokenizer.convert_ids_to_tokens(id) for id in tokens_input])
            print([tokenizer.convert_ids_to_tokens(id) for id in input_ids])
            print(tokenizer.decode(input_ids))
        #################################
        padding_length_a = max_length - len(tokens_input)
        input_ids = [input_ids + ([pad_token] * padding_length_a) for _ in range(num_dependencies)] 
        token_ids = [token_ids + ([0] * padding_length_a) for _ in range(num_dependencies)] 
        input_attention_mask = [[1] * len(tokens_input) + ([0] * padding_length_a) for _ in range(num_dependencies)]
