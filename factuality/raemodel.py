...
class RAEBaseModel(LongformerPreTrainedModel):
    ...
    def get_embeddings(self, tree, bsz, device='cuda'):
        ...
        input_ids = tree.input_ids[:1,...]
        attention_mask = tree.input_attention_mask[:1,...]
        token_type_ids = tree.token_ids[:1,...]
        transformer_outputs = self.bert(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)

        outputs = transformer_outputs[0]
        outputs = self.dropout(outputs)
        outputs = outputs.view((-1, outputs.size(-1))) # len, emb_dim
        ...
        arc_label_lengths = tree.arc_label_lengths.view(-1)
    ...
    def get_both_embeddings(self, input_tree, context_tree, bsz, device='cuda'):
        ...
        input_embeddings = self.get_embeddings(input_tree,bsz, device=device) # m * 1 * dim
    ...
    def forward(self, features, bsz=4, device='cuda'):
        ...
        final_embeddings = self.get_embeddings(context_tree, bsz, device=device) # n * 1 * 2dim

        logits_all = self.dep_label_classifier(final_embeddings).squeeze(1) # n * 1 * 2

        # each_context_arc_scores, _ = torch.max(logits_all, dim=1) # n * 1 *  2

        # example_avg_score = torch.mean(logits_all, dim=0) # 1 * 2

        return logits_all


class RAEDocModel(LongformerPreTrainedModel):
    def __init__(self, config):
        super(RAEDocModel, self).__init__(config)
        self.bert = LongformerModel.from_pretrained('allenai/longformer-base-4096')
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.dep_label_classifier = nn.Linear(3 * config.hidden_size, 2)
    ...
    def forward(self, features, bsz=4, device='cuda'):
        ...
        logits_all = self.dep_label_classifier(final_embeddings) # n * 1 * 2
...
class InputFeatures(object):
    ...
    def to(self, device):
        ...
        self.num_dependencies = self.num_dependencies.to(device)
...
def load_and_cache_examples(data_path, feature_path, max_length, tokenizer, shuffle):
    ...
    for feature in features:
        if feature is None:
            continue
        yield feature, feature['context_tree'].sentence_label.squeeze(1)



    
class RAEArcModel(LongformerPreTrainedModel):
    def __init__(self, config):
        super(RAEArcModel, self).__init__(config)
        self.bert = LongformerModel.from_pretrained('allenai/longformer-base-4096')
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def get_context_embeddings(self, tree, bsz, device='cuda'):
        ...
        for i in range(int(batch_size/bsz)+1):
            ...
            batch_arc_outputs = self.bert(batch_arcs, attention_mask=batch_arc_attention)
    ...
    def get_input_embeddings(self, tree, bsz, device='cuda'):
        ...
        child_embeddings = torch.stack([torch.mean(outputs[tree.child_start_indices[i,].item():tree.child_end_indices[i,].item(),:], dim=0) for i in range(batch_size)], dim=0)
        child_embeddings = child_embeddings.view(batch_size, -1, child_embeddings.size(-1))
        child_embeddings = torch.sum(child_embeddings, dim=0, keepdim=True)
        ...
        arc_outputs = torch.zeros((1, 768)).to(device)
        ...
        for i in range(int(batch_size/bsz)+1):
            ...
            batch_arc_outputs = batch_arc_outputs[1]
            batch_arc_outputs = self.dropout(batch_arc_outputs)
            batch_arc_outputs = torch.sum(batch_arc_outputs, dim=0, keepdim=True)
            
            arc_outputs += batch_arc_outputs
        arc_outputs /= batch_size
        
        arc_outputs = arc_outputs.view(1, -1, arc_outputs.size(-1))  # 1 * 1 * emb_dim
       
        final_embeddings = torch.cat([child_embeddings, head_embeddings, arc_outputs], dim=2) # 1 * 1 * (3 * 768)
        print(final_embeddings.shape)
        return final_embeddings
    
    def get_both_embeddings(self, input_embeddings, context_embeddings):
        
        
        context_arc_num = context_embeddings.shape[0]
        input_embeddings = input_embeddings.expand(context_arc_num, -1, -1)
        
        assert context_embeddings.shape == input_embeddings.shape

        final_embeddings = torch.cat([input_embeddings, context_embeddings], dim=-1) # n * 1 * 2dim
        return final_embeddings
    ...
    def forward(self, features, bsz=4, device='cuda'):
        ...
        input_embeddings = self.get_input_embeddings(input_tree, bsz, device)
        context_embeddings = self.get_context_embeddings(context_tree, bsz, device=device) # n * 1 * 2dim
        final_embeddings = self.get_both_embeddings(input_embeddings, context_embeddings)
